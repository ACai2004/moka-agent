...
--- 用餐信息 ---
餐厅：川·隐味小馆
氛围：暖色灯光、复古海报、藤编灯具（如有匹配的餐厅信息）
服务：主动添茶、稳定上菜节奏（如有匹配的餐厅信息）
时间：周五 19:20
人数：3 人
用餐时长：2 小时 15 分钟
菜品：
  · 麻婆豆腐 ×1 —— 经典川菜、麻辣鲜香，下饭
  · 打抛饭 ×1（辣度：不可免辣，备注：加牛肉）—— 经典泰式、酸辣开胃，招牌必点
  · 冰柠檬水 ×2
当日天气：大雨
临近节日：端午节
```

> **注意**：Raw Facts 中的菜品信息保留了原始小票的细节（数量、辣度要求、备注等），并自动追加了菜品特点（features）和体验方向（experienceTags，来自菜品知识库匹配结果）。餐厅氛围和服务信息来自餐厅资料库。这些信息只是背景知识，AI 不应主动逐项覆盖，具体使用方式由 Conversation Planner 控制（见第 5.2 节）。
...
示例：
```
- 推测：可能存在多人聚餐场景，用户可能更关注整体氛围
- 推测：当天是端午节，这顿饭可能有节日聚餐的背景
- 参考：用户偏好友聚餐，打抛饭和多人份菜品印证了这一倾向
```

> **置信度标签映射**：`MEDIUM` → "推测"（可信度较高，可作为对话指导），`LOW` → "参考"（可信度一般，仅供参考）。不允许出现 HIGH（第 13 节禁止项）。
